...
SELECT
  o.origin_id,
  o.node AS source_node,
  d.destination_id,
  d.node AS destination_node,
  rank() OVER (PARTITION BY origin_id ORDER BY ROUND(SUM(x.cost)::numeric, 2)) as destination_rank,
  d.destination_name,
...
FROM
  pgr_Dijkstra(
    ...
    (SELECT array_agg(node) FROM (SELECT DISTINCT node FROM origins) as f),
...
GROUP BY o.origin_id, o.node, d.destination_id, d.node, d.destination_name
ORDER BY o.origin_id, SUM(x.cost);